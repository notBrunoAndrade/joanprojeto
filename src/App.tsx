...
function App() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (editingUserId) {
      ...
      await api.put(`clientes`, updatedUser);
      setUsers(
  ...
  const handleDelete = async (id: string) => {
    try {
      await api.delete(`/clientes/${id}`); // Realiza a requisição DELETE
      // Atualiza a lista de usuários, filtrando o usuário que foi deletado
            ...
            <tr key={clientes.id}>
              ...
              <td className="edit-delete">
                <button onClick={() => handleDelete(clientes.id)}>
                  <GoXCircleFill
                    className="i"
                    style={{ color: "red", fontSize: 28, cursor: "pointer" }}
                  />
                </button>
                <button onClick={() => handleEdit(clientes)}>
                  <GoPencil
                    className="i"
                    style={{ color: "green", fontSize: 28, cursor: "pointer" }}
                  />
                </button>
              </td>
            </tr>
